...
- **Detección automática**: Si el usuario inicia sesión y su UID no tiene perfil en Firestore, muestra un flujo guiado en vez de una tarjeta desalineada. Explica que se va a crear su perfil y la organización.
- **Aviso explícito**: Informa que se creará una nueva organización y que el usuario será el administrador principal (`role: admin`, `active: true`, `isMaintenanceLead: true`).
...
- **Modelo sencillo** (recomendado al inicio): el perfil contiene un solo `organizationId`; un mismo email puede registrarse varias veces (una por organización). Simplifica queries y reglas de seguridad.
- **Modelo avanzado**: tabla de membresías (`memberships`) con `userId`, `organizationId`, rol y estado. Permite que el mismo UID pertenezca a varias organizaciones y cambiar de organización con un selector en la UI. Implica endurecer reglas de Firestore para validar `organizationId` en cada consulta.
...
## 7) UX recomendada en la página de registro
