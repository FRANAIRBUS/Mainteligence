...
String sanitizeDeviceKey(const String& value) {
  String output;
  ...
  if (output.length() > 47) output = output.substring(0, 47);
  return output;
}

String defaultDeviceKey() {
  char suffix[16];
  snprintf(suffix, sizeof(suffix), "ESP%06lX", static_cast<unsigned long>(appChipId() & 0xFFFFFFUL));
  String base = trimCopy(String(config().hostName));
  if (!base.length()) base = "LYENA";
  String key = sanitizeDeviceKey(base + "-" + String(suffix));
  if (key.length() < 4) key = String("LYENA-") + String(suffix);
  if (key.length() > 47) key = key.substring(0, 47);
  return key;
}
...
bool bootstrapDevice() {
  ...
  String deviceKey = trimCopy(String(config().iotDeviceKey));
  ...
  if (!organizationId.length() || !bootstrapToken.length() || !bootstrapUrl.length()) {
    setCloudState("config-miss", "missing bootstrap fields");
    return false;
  }
  if (!assetId.length() && !deviceKey.length()) {
    deviceKey = defaultDeviceKey();
    if (!config().iotDeviceKey[0]) {
      copyText(mutableConfig().iotDeviceKey, deviceKey);
      saveConfigToFile();
      addLog("Cloud IoT generated deviceKey: %s", deviceKey.c_str());
    }
  }
