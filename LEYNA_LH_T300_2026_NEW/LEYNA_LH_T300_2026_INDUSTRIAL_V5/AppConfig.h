...
static constexpr uint16_t kConfigSchemaVersion = 7;
...
enum TelemetryMode : uint8_t {
  TELEMETRY_OFF = 0,
  TELEMETRY_JSON = 1,
  TELEMETRY_LEGACY_FORM = 2,
  TELEMETRY_MAINTELLIGENCE = 3,
};
...
struct AppConfigData {
  ...
  char telemetryEndpoint[96];
  char telemetryApiKey[48];
  char telemetryUser[32];
  char telemetryPass[32];
  char telemetryDbName[32];
  char telemetryLocation[40];
  char telemetryDeviceName[40];
  char telemetryBearer[96];
  bool telemetryAllowInsecureTls;
...
struct AppRuntimeData {
  ...
  int32_t wifiRssi;
  uint16_t lastTelemetryCode;
  uint16_t lastCloudCode;
  uint32_t cloudWorkerStackHighWater;
  char ipAddress[20];
  char connectedSsid[40];
  char apSsid[40];
  char lastTelemetryStatus[24];
  char lastTelemetryError[96];
