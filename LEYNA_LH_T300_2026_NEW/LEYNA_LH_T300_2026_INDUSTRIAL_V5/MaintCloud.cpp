...
void fillReportedState(JsonObject reportedState, const String& applyStatus, const String& applyMessage, long appliedDesiredVersion) {
  ...
  reportedState["firmwareVersion"] = config().iotFirmwareVersion[0] ? config().iotFirmwareVersion : "lh-t300-v5";
...
bool postJson(const String& url, const String& body, DynamicJsonDocument& responseDoc, int& httpCode, bool signRequest) {
  ...
  addLog("Cloud IoT response bytes: %u", static_cast<unsigned>(responseBody.length()));
  http.end();
  ...
  if (responseBody.length()) {
    ...
    if (error) {
      ...
      addLog("Cloud IoT non-JSON response preview: %s", preview.c_str());
    }
  }
  if (httpCode <= 0) {
    const String driverMessage = HTTPClient::errorToString(httpCode);
    setCloudState("http-error", responseBody.length() ? responseBody : driverMessage);
  ...
  if (httpCode < 200 || httpCode >= 300) {
    ...
    if (!message.length()) message = responseBody;
    if (!message.length()) message = String("HTTP ") + httpCode;
    if (responseBody.length()) {
      String preview = responseBody;
      if (preview.length() > 240) preview = preview.substring(0, 240) + "...";
      addLog("Cloud IoT API error body: %s", preview.c_str());
    }
...
bool applyDesiredState(JsonObjectConst desiredState, String& applyStatus, String& applyMessage, long& appliedVersion) {
  ...
  if (desiredState.containsKey("power") && !desiredState["power"].isNull()) {
    const bool requestedPower = desiredState["power"].as<bool>();
    if (!requestedPower) {
    ...
    } else if (mutableConfig().workMode == WORK_DISABLED) {
      mutableConfig().workMode = WORK_THERMOSTAT;
      configChanged = true;
    }
  }
  ...
  if (!relayObject.isNull() && relayObject.size() > 0) {
...
bool bootstrapDevice() {
  ...
  payloadDoc["firmwareVersion"] = config().iotFirmwareVersion[0] ? config().iotFirmwareVersion : "lh-t300-v5";
