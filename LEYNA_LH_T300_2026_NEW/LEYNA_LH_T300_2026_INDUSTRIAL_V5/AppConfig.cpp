...
void resetConfigToDefaults(AppConfigData& config) {
  ...
  config.telemetryMode = TELEMETRY_OFF;
  config.telemetryPeriodSec = 30;
  config.telemetryAllowInsecureTls = false;
  config.allowInsecureTls = false;
  setDefaultText(config.telemetryDeviceName, sizeof(config.telemetryDeviceName), "T300-V5");
  setDefaultText(config.iotFirmwareVersion, sizeof(config.iotFirmwareVersion), "lh-t300-v5");
...
void sanitizeConfig(AppConfigData& config) {
  ...
  if (!config.iotFirmwareVersion[0]) setDefaultText(config.iotFirmwareVersion, sizeof(config.iotFirmwareVersion), "lh-t300-v5");
  ...
  if (config.telemetryMode == TELEMETRY_MAINTELLIGENCE) {
    config.enableCloudIot = true;
    config.telemetryMode = TELEMETRY_OFF;
  }

  config.workMode = clampValue<uint8_t>(config.workMode, WORK_DISABLED, WORK_MANUAL);
  ...
  config.telemetryPeriodSec = clampValue<uint16_t>(config.telemetryPeriodSec, 5, 3600);
  ...
  config.telemetryMode = clampValue<uint8_t>(config.telemetryMode, TELEMETRY_OFF, TELEMETRY_LEGACY_FORM);
  ...
  for (size_t index = 0; index < kChannelCount; ++index) {
    config.sensorTypes[index] = clampValue<uint8_t>(config.sensorTypes[index], SENSOR_NONE, SENSOR_INTERNAL);
  }
}
...
void serializeConfig(JsonObject json, const AppConfigData& config) {
  ...
  json["enableTelemetry"] = config.enableTelemetry;
  ...
  json["telemetryMode"] = config.telemetryMode;
  json["telemetryEndpoint"] = config.telemetryEndpoint;
  json["telemetryApiKey"] = config.telemetryApiKey;
  json["telemetryUser"] = config.telemetryUser;
  json["telemetryPass"] = config.telemetryPass;
  json["telemetryDbName"] = config.telemetryDbName;
  json["telemetryLocation"] = config.telemetryLocation;
  json["telemetryDeviceName"] = config.telemetryDeviceName;
  json["telemetryBearer"] = config.telemetryBearer;
  json["telemetryAllowInsecureTls"] = config.telemetryAllowInsecureTls;
  ...
  json["telemetryPeriodSec"] = config.telemetryPeriodSec;
...
void deserializeConfig(JsonObjectConst json, AppConfigData& config) {
  ...
  config.enableTelemetry = json["enableTelemetry"] | config.enableTelemetry;
  ...
  config.telemetryMode = json["telemetryMode"] | config.telemetryMode;
  copyText(config.telemetryEndpoint, json["telemetryEndpoint"] | String(config.telemetryEndpoint));
  copyText(config.telemetryApiKey, json["telemetryApiKey"] | String(config.telemetryApiKey));
  copyText(config.telemetryUser, json["telemetryUser"] | String(config.telemetryUser));
  copyText(config.telemetryPass, json["telemetryPass"] | String(config.telemetryPass));
  copyText(config.telemetryDbName, json["telemetryDbName"] | String(config.telemetryDbName));
  copyText(config.telemetryLocation, json["telemetryLocation"] | String(config.telemetryLocation));
  copyText(config.telemetryDeviceName, json["telemetryDeviceName"] | String(config.telemetryDeviceName));
  copyText(config.telemetryBearer, json["telemetryBearer"] | String(config.telemetryBearer));
  config.telemetryAllowInsecureTls = json["telemetryAllowInsecureTls"] | config.telemetryAllowInsecureTls;
  ...
  config.telemetryPeriodSec = json["telemetryPeriodSec"] | config.telemetryPeriodSec;
  ...
  for (size_t index = 0; index < kChannelCount; ++index) {
    ...
    if (index < modbusTempRegisters.size()) config.modbusTempRegisters[index] = modbusTempRegisters[index].as<uint16_t>();
    if (index < modbusMirrorTempRegisters.size()) config.modbusMirrorTempRegisters[index] = modbusMirrorTempRegisters[index].as<uint16_t>();
    ...
    if (index < modbusHumRegisters.size() && index < kHumidityChannelCount) {
      config.modbusHumRegisters[index] = modbusHumRegisters[index].as<uint16_t>();
    }
    if (index < modbusMirrorHumRegisters.size() && index < kHumidityChannelCount) {
      config.modbusMirrorHumRegisters[index] = modbusMirrorHumRegisters[index].as<uint16_t>();
    }
